 ...
 * Reparto entities. Transaction control of the save(), update() and delete()
...
public class RepartoDAO extends BaseHibernateDAO {
	...
	public Reparto findById(RepartoId id) {
		...
		try {
			Reparto instance = (Reparto) getSession().get("Reparto", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Reparto instance) {
		...
		try {
			List results = getSession().createCriteria("Reparto").add(
	...
	public Reparto merge(Reparto detachedInstance) {
		...
		try {
			Reparto result = (Reparto) getSession().merge(detachedInstance);
