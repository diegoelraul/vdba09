 ...
 * Compra entities. Transaction control of the save(), update() and delete()
...
public class CompraDAO extends BaseHibernateDAO  {
	...
	public static final String NUM_TARJETA = "numTarjeta";
	...
	public Compra findById(CompraId id) {
		...
		try {
			Compra instance = (Compra) getSession().get("Compra", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Compra instance) {
		...
		try {
			List results = getSession().createCriteria("Compra").add(
	...
	@SuppressWarnings("unchecked")
	public List findByFormaPago(Object formaPago) {
		return findByProperty(FORMA_PAGO, formaPago);
	}

	@SuppressWarnings("unchecked")
	public List findByNumTarjeta(Object numTarjeta) {
		return findByProperty(NUM_TARJETA, numTarjeta);
	}
	...
	public Compra merge(Compra detachedInstance) {
		...
		try {
			Compra result = (Compra) getSession().merge(detachedInstance);
